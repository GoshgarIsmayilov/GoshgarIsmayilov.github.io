...
// Render the gallery grid from the bundled data file (gallery-data.js).
function loadGallery() {
  const container = document.getElementById('gallery-container');
  if (!container) return;

  if (typeof GALLERY_DATA === 'undefined') {
    console.error('GALLERY_DATA not found — is gallery-data.js loaded before scripts.js?');
    return;
  }

  GALLERY_DATA.photos.forEach(photo => {
...
// Fetch and increment the site's visit count using a free public counter API.
// Fails silently (leaves the element empty) if the API is unreachable.
function loadVisitCount() {
  const el = document.getElementById('visit-count');
  if (!el) return;

  fetch('https://abacus.jasoncameron.dev/hit/goshgarismayilov.github.io/homepage-visits')
    .then(response => {
      if (!response.ok) {
...
// Load publications from the bundled data file (publications-data.js).
// Using a plain <script> instead of fetch() means this also works when the
// page is opened directly as a file:// URL, where fetch() of local files is
// blocked by the browser.
function loadPublications() {
  if (typeof PUBLICATIONS_DATA === 'undefined') {
    console.error('PUBLICATIONS_DATA not found — is publications-data.js loaded before scripts.js?');
    displayFallbackPublications();
    return;
  }
  allPublications = PUBLICATIONS_DATA.publications;
...
function createPublicationElement(publication) {
  ...
  // Create thumbnail (only if one is provided)
  ...
  if (publication.links) {
    ...
    Object.keys(publication.links).forEach(key => {
      ...
      link.textContent = `[${linkLabels[key] || key}]`;
